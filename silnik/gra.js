...
(function () {
  ...
  const STADIA = [
    { food: 4, dl: 90,  zoom: 0.884 },
    { food: 5, dl: 106, zoom: 0.816 },
    { food: 5, dl: 124, zoom: 0.748 },
    { food: 6, dl: 146, zoom: 0.680 },
    { food: 6, dl: 172, zoom: 0.612 },
    { food: 7, dl: 205, zoom: 0.544 },
    { food: 7, dl: 240, zoom: 0.490 },
    { food: 0, dl: 285, zoom: 0.435 }   // L8 dorosła, koniec wzrostu
  ];

  const SWIAT_SZER = 2600;
  ...
  const stan = {
    ...
    krople: [], plusk: [], kaluze: [], deszczSila: 0, wiatr: 0, gruntCss: 0,
    czas: 0, iskry: []
  };
  ...
  function nowyOwad(zaKadrem) {
    ...
    /* trzymamy owady w głębi świata: przy krawędzi odbijamy je do środka */
    const margines = 260;
    if (x < margines) x = margines + Math.random() * 160;
    if (x > SWIAT_SZER - margines) x = SWIAT_SZER - margines - Math.random() * 160;
    const lata = t.ruch.startsWith('lot') || t.ruch === 'lot-chwiej';
    const naZiemi = !lata;
    return {
      typ, x: Math.max(160, Math.min(SWIAT_SZER - 160, x)),
  ...
  function ruchOwada(o, dt) {
    ...
    /* przy krawędzi świata owad zawraca do środka, zamiast się o nią kleić */
    const brzeg = 200;
    if (o.x < brzeg) { o.x += 40 * dt; if (o.vx < 0) o.vx = -o.vx; o.kierunek = 1; }
    if (o.x > SWIAT_SZER - brzeg) { o.x -= 40 * dt; if (o.vx > 0) o.vx = -o.vx; o.kierunek = -1; }
    o.x = Math.max(120, Math.min(SWIAT_SZER - 120, o.x));
  }
  ...
  /* --- ooteka: epilog, dorosła samica składa kokon, wychodzą maleństwa -
     Kokon nie pojawia się od razu: samica wyciska pianę, kokon rośnie w
     oczach bańka po bańce, piana zastyga w żeberka, a dopiero potem z
     kokonu wychodzą maleństwa, jedno po drugim. */
  const OOT_FORM = 3.2;        // ile sekund powstaje kokon
  const OOT_WYKL = 6.3;        // kiedy zaczyna się wykluwanie

  function rozpocznijOoteke() {
    const m = stan.modliszka;
    /* piana wychodzi spod odwłoka, czyli z tyłu za modliszką; kokon ma
       mniej wiecej jedna trzecia dlugosci doroslej modliszki, jak w naturze */
    stan.ooteka = { x: m.x - m.kierunek * m.dlugosc * 0.3, y: gruntY - m.dlugosc * 0.46,
                    rx0: m.dlugosc * 0.19, ry0: m.dlugosc * 0.11,
                    t: 0, narost: 0, male: [], banki: [] };
    karta('ikona:ooteka');
    m.cel = null; m.owadCel = null;
  }
  function aktualizujOoteke(dt) {
    const o = stan.ooteka; o.t += dt;
    o.narost = Math.min(1, o.t / OOT_FORM);
    /* bańki piany, póki kokon się buduje */
    if (o.narost < 1 && Math.random() < dt * 16) {
      o.banki.push({ dx: (Math.random() - 0.5) * o.rx0 * 1.5 * (0.4 + o.narost),
                     dy: (Math.random() - 0.5) * o.ry0 * 1.4,
                     r: o.ry0 * (0.2 + Math.random() * 0.22), zyc: 0 });
    }
    o.banki.forEach(b => { b.zyc += dt; });
    o.banki = o.banki.filter(b => b.zyc < 1.5);
    if (o.t > OOT_WYKL && o.male.length === 0) {
      for (let i = 0; i < 12; i++) {
        /* ujemny czas życia: maleństwa wychodzą kolejno, nie wszystkie naraz */
        o.male.push({ x: o.x + (Math.random() - 0.5) * o.rx0 * 1.4, vx: (Math.random() - 0.5) * 62,
                      faza: Math.random() * 6, zyc: -i * 0.2 });
      }
      if (onEpilog) onEpilog();          // podświetl domek: to koniec
    }
    o.male.forEach(mm => {
      mm.zyc += dt;
      if (mm.zyc > 0) { mm.x += mm.vx * dt * Math.min(1, mm.zyc); mm.faza += dt; }
    });
  }
  ...
  function aktualizujZwyciestwo(dt) {
    ...
    if (stan.ooteka) aktualizujOoteke(dt);
    /* finał oglądamy z bliska: kadr dojeżdża, żeby było dobrze widać, jak
       powstaje kokon i jak wychodzą z niego maleństwa */
    m.zoom += (0.62 - m.zoom) * Math.min(1, dt * 0.7);
    /* kamera trzyma modliszkę i kokon */
    const cel = stan.ooteka ? (stan.ooteka.x + m.x) / 2 : m.x;
    const widoczne = SZER / m.zoom;
  ...
  function wejdzWolne() {
    const m = stan.modliszka;
    m.zoomCel = STADIA[STADIA.length - 1].zoom;      // wracamy do zwykłego kadru
  ...
  function rysujOoteke() {
    const o = stan.ooteka;
    const n = o.narost === undefined ? 1 : o.narost;
    const rxP = o.rx0 || 48, ryP = o.ry0 || 28;
    /* łodyga, do której przyklejony jest kokon */
    ctx.strokeStyle = '#7d6a3f'; ctx.lineCap = 'round'; ctx.lineWidth = Math.max(3, ryP * 0.22);
    ctx.beginPath();
    ctx.moveTo(o.x - ryP * 0.4, gruntY + 8);
    ctx.quadraticCurveTo(o.x - ryP * 0.2, o.y + ryP, o.x, o.y - ryP * 0.6);
    ctx.stroke();
    /* kokon rośnie: najpierw mała kropla piany, potem pełna ooteka */
    if (n > 0.02) {
      const rx = rxP * (0.22 + 0.78 * n), ry = ryP * (0.3 + 0.7 * n);
      const drg = n < 1 ? Math.sin(o.t * 16) * ryP * 0.05 : 0;   // świeża piana drga
      ctx.fillStyle = '#e6d3a0'; ctx.strokeStyle = '#b79a63'; ctx.lineWidth = 2;
      ctx.beginPath(); ctx.ellipse(o.x + drg, o.y, rx, ry, 0, 0, 7); ctx.fill(); ctx.stroke();
      /* poprzeczne żeberka pojawiają się, gdy piana zastyga */
      ctx.strokeStyle = 'rgba(150,120,70,' + (0.5 * n) + ')'; ctx.lineWidth = 1.4;
      for (let i = -3; i <= 3; i++) {
        const dx = i * rx * 0.26;
        if (Math.abs(dx) > rx - 2) continue;
        const h = ry * Math.sqrt(Math.max(0, 1 - (dx / rx) * (dx / rx)));
        ctx.beginPath(); ctx.moveTo(o.x + dx, o.y - h); ctx.lineTo(o.x + dx, o.y + h); ctx.stroke();
      }
      if (n < 1) {                                            // mokry połysk świeżej piany
        ctx.fillStyle = 'rgba(255,255,255,0.4)';
        ctx.beginPath(); ctx.ellipse(o.x - rx * 0.3, o.y - ry * 0.4, rx * 0.3, ry * 0.28, 0, 0, 7); ctx.fill();
      }
    }
    /* bańki piany wyciskane przez samicę */
    o.banki.forEach(b => {
      ctx.globalAlpha = Math.max(0, 1 - b.zyc / 1.5) * 0.85;
      ctx.fillStyle = '#f5e9c8'; ctx.strokeStyle = 'rgba(180,150,100,0.6)'; ctx.lineWidth = 0.8;
      ctx.beginPath(); ctx.arc(o.x + b.dx, o.y + b.dy - b.zyc * 5, b.r, 0, 7); ctx.fill(); ctx.stroke();
    });
    ctx.globalAlpha = 1;
    /* maleństwa L1 wychodzą z kokonu i schodzą na ziemię */
    o.male.forEach(mm => {
      if (mm.zyc <= 0) return;
      const s = Math.min(1, mm.zyc / 0.9);
      const y = o.y + ryP + (gruntY - o.y - ryP) * s;
      ctx.globalAlpha = Math.min(1, mm.zyc * 3);
      Modliszka.rysuj(ctx, { x: mm.x, y, dlugosc: 74, stadium: 1, cute: true,
  ...
  /* --- deszcz: krople, wiatr, plusk i kałuże --------------------------
     Krople mają różne prędkości i własny znoś, a wiatr wolno błądzi, więc
     deszcz nie leci równo w jedną stronę. Spadając na ziemię zostawiają
     plusk, a na gruncie zbierają się kałuże, które po deszczu powoli
     wsiąkają. */
  function nowaKropla(gdziekolwiek) {
    const W = stan.szerEkranuCss, H = stan.wysEkranuCss;
    return {
      x: Math.random() * (W + 80) - 40,
      y: gdziekolwiek ? Math.random() * H : -20 - Math.random() * 120,
      v: 170 + Math.random() * 180,                 // każda kropla spada inaczej
      wiatr: (Math.random() - 0.5) * 0.3,           // własny znoś kropli
      dl: 9 + Math.random() * 13,
      gruba: 1.2 + Math.random() * 1.3,
      a: 0.34 + Math.random() * 0.3
    };
  }

  function aktualizujDeszcz(dt) {
    const pada = stan.pogoda === 'deszcz';
    /* deszcz narasta i ustaje łagodnie, nie włącza się jednym pstryknięciem */
    stan.deszczSila += ((pada ? 1 : 0) - stan.deszczSila) * Math.min(1, dt * (pada ? 0.6 : 0.4));
    /* wiatr błądzi: dwie wolne fale o różnych okresach */
    stan.wiatr = Math.sin(stan.czas * 0.11) * 0.5 + Math.sin(stan.czas * 0.043 + 1.7) * 0.3;
    /* wysokość gruntu w pikselach ekranu, żeby krople miały gdzie pluskać */
    const m = stan.modliszka;
    const sx = stan.skalaEkranu * m.zoom;
    const ty = stan.marginY + stan.skalaEkranu * gruntY * (1 - m.zoom);
    stan.gruntCss = (ty + sx * gruntY) / dpr;

    const W = stan.szerEkranuCss;
    const ile = Math.round(stan.deszczSila * 52);
    while (stan.krople.length < ile) stan.krople.push(nowaKropla(stan.krople.length < 10));
    while (stan.krople.length > ile) stan.krople.pop();
    const dno = Math.min(stan.wysEkranuCss + 20, stan.gruntCss + 26);
    stan.krople.forEach(k => {
      k.y += k.v * dt;
      k.x += k.v * dt * (stan.wiatr * 0.5 + k.wiatr);
      if (k.y > dno) {
        if (stan.plusk.length < 24) stan.plusk.push({ x: k.x, y: dno - Math.random() * 14, t: 0 });
        Object.assign(k, nowaKropla(false));
      }
      if (k.x < -40) k.x += W + 80; else if (k.x > W + 40) k.x -= W + 80;
    });
    stan.plusk.forEach(p => { p.t += dt; });
    stan.plusk = stan.plusk.filter(p => p.t < 0.45);

    aktualizujKaluze(dt, pada);
  }

  function aktualizujKaluze(dt, pada) {
    if (pada && stan.deszczSila > 0.25 && stan.kaluze.length < 9 && Math.random() < dt * 1.6) {
      const widoczne = SZER / stan.modliszka.zoom;
      const x = stan.kamera + (Math.random() - 0.5) * widoczne * 1.3;
      if (x > 90 && x < SWIAT_SZER - 90 && !stan.kaluze.some(k => Math.abs(k.x - x) < 80)) {
        stan.kaluze.push({ x, r: 26 + Math.random() * 50, poziom: 0, faza: Math.random() * 6 });
      }
    }
    stan.kaluze.forEach(k => {
      k.faza += dt;
      /* w deszczu kałuża rośnie szybko, potem wsiąka powoli */
      k.poziom = Math.min(1, k.poziom + (pada ? dt * 0.32 : -dt * 0.05));
    });
    stan.kaluze = stan.kaluze.filter(k => k.poziom > 0);
  }

  /* kałuże rysujemy w świecie, tuż na gruncie, pod owadami i modliszką */
  function rysujKaluze() {
    if (!stan.kaluze.length) return;
    stan.kaluze.forEach(k => {
      const a = Math.min(1, k.poziom);
      const rx = k.r * (0.5 + 0.5 * a), ry = k.r * 0.2;
      const g = ctx.createLinearGradient(0, gruntY + 6 - ry, 0, gruntY + 6 + ry);
      g.addColorStop(0, 'rgba(198,222,246,' + (0.55 * a) + ')');
      g.addColorStop(1, 'rgba(84,116,150,' + (0.45 * a) + ')');
      ctx.fillStyle = g;
      ctx.strokeStyle = 'rgba(255,255,255,' + (0.3 * a) + ')'; ctx.lineWidth = 1.5;
      ctx.beginPath(); ctx.ellipse(k.x, gruntY + 6, rx, ry, 0, 0, 7); ctx.fill(); ctx.stroke();
      /* kręgi po kroplach, tylko gdy naprawdę pada */
      if (stan.deszczSila > 0.3) {
        const p = (k.faza * 0.8) % 1;
        ctx.strokeStyle = 'rgba(255,255,255,' + (0.35 * a * (1 - p)) + ')'; ctx.lineWidth = 1;
        ctx.beginPath(); ctx.ellipse(k.x, gruntY + 6, rx * 0.45 * p + 3, ry * 0.5 * p + 1.5, 0, 0, 7); ctx.stroke();
      }
    });
  }
  ...
  function rysujPogoda() {
    const W = stan.szerEkranuCss, H = stan.wysEkranuCss;
    ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
    if (stan.deszczSila > 0.02) {
      const s = stan.deszczSila;
      ctx.fillStyle = 'rgba(60,80,110,' + (0.10 * s) + ')'; ctx.fillRect(0, 0, W, H);
      ctx.lineCap = 'round';
      stan.krople.forEach(k => {
        const kat = stan.wiatr * 0.5 + k.wiatr;
        ctx.strokeStyle = 'rgba(192,212,240,' + (k.a * s) + ')';
        ctx.lineWidth = k.gruba;
        ctx.beginPath();
        ctx.moveTo(k.x, k.y); ctx.lineTo(k.x - kat * k.dl, k.y - k.dl);
        ctx.stroke();
      });
      stan.plusk.forEach(p => {
        const t = p.t / 0.45;
        ctx.strokeStyle = 'rgba(214,234,255,' + (0.5 * (1 - t) * s) + ')'; ctx.lineWidth = 1.2;
        ctx.beginPath(); ctx.ellipse(p.x, p.y, 2 + t * 9, 1 + t * 3, 0, 0, 7); ctx.stroke();
      });
    }
    if (stan.pogoda === 'slonce') {
  ...
  function startGry(gatunek) {
    ...
    stan.iskry = [];
    stan.pogoda = 'normalna'; stan.pogodaDo = stan.czas + 14;
    stan.krople = []; stan.plusk = []; stan.kaluze = []; stan.deszczSila = 0;
    stan.faza = 'gra';
